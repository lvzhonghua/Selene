...
namespace Selene
{
    public partial class TreeTest01 : Form
    {
        public TreeTest01()
        {
            InitializeComponent();
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            DrawTree(allPersons, plMain.CreateGraphics());
        }
        ...
        private void CalcXY(List<Person> persons)
        {
            ...
            if (cbkRetract.Checked)
            {
                CalcRetract(allPersons);
            }
            
        }

        private void CalcRetract(List<Person> persons)
        {
            int maxX = persons.Max(p => p.X);
            int maxY = persons.Max(p => p.Y);

            int minX = persons.Min(p => p.X);
            int minY = persons.Min(p => p.Y);

            for (int x = minX; x < maxX; x++)
            {
                for (int y = minY; y < maxY; y++)
                {
                    Person person = GetPerson(x, y);
                    if (person != null)
                    {
                        continue;
                    }
                    ...
                    int verCount = GetXLastVerCount(x, y, maxX);
                    if (verCount == 1)
                    {
                        var lastPerson = GetXLast(x, y, maxX);
                        lastPerson.X = x;
                        ...
                        var parentPerson = allPersons.First(p => p.Id == lastPerson.Pid);

                        if (x < parentPerson.X)
                        {
                            lastPerson.X = parentPerson.X;
                        }
                    }
                    else
                    {
                        var emptyVerCount = GetEmptyVerCount(x, y, maxY);
                        if (verCount > 0 && verCount <= emptyVerCount)
                        {
                            var lastPerson = GetXLast(x, y, maxX);
                            var diffX = lastPerson.X - x;

                            if (lastPerson.Childrens.Count > 0)
                            {
                                var childPersons = new List<Person>();
                                GetPersonAll(lastPerson, childPersons);
                                childPersons.Remove(lastPerson);

                                CalcRetract(childPersons);
                            }

                            UpdatePersonChild(lastPerson, diffX);
        ...
        private void UpdatePersonChild(Person person, int diffX)
        {
            List<Person> persons = new List<Person>();
            GetPersonAll(person, persons);

            persons.ForEach(p => p.X -= diffX);
        }



        private void GetPersonAll(Person person, List<Person> persons)
        {
            if (persons.Count == 0)
            {
                persons.Add(person);
            ...
            if (person.Childrens.Count > 0)
            {
                ...
                {
                    persons.Add(item);
                    GetPersonAll(item, persons);
                }
            }
        }

        private int GetEmptyVerCount(int x, int y, int maxY)
        {
            int emptyCount = 0;
            var person = GetPerson(x, y);
            while (person == null && y <= maxY)
            {
                y++;
                emptyCount++;
                person = GetPerson(x, y);
            }
            return emptyCount;
        }

        private Person GetPerson(int x, int y)
        {
            return allPersons.FirstOrDefault(p => p.X == x && p.Y == y);
        }

        private Person GetXLast(int x, int y, int maxX)
        {
            var person = GetPerson(x, y);
            while (person == null && x < maxX)
            {
                x++;
                person = GetPerson(x, y);
            }
        ...
        //获取最后一个的纵向的个数(必须父子关系)
        private int GetXLastVerCount(int x, int y, int maxX)
        {
            var person = GetXLast(x, y, maxX);
            if (person == null)
            {
                return 0;
            }
            return ChildDep(person, 0);
        }

        private int ChildDep(Person person, int dep)
        {
            if (person.Childrens.Count <= 0)
            {
                return 1;
            ...
            dep++;
            dep += ChildDep(person.Childrens.First(), dep);
            return dep;
        }
        ...
        {
            allPersons.ForEach(p => p.HorCount = CalcChildrenCount(p));
            ...
            list.ForEach(l =>
            {
                ...
                l.Value.ForEach(p =>
                {
                    if (upPerson != null)
                    {
                        //p.X = upPerson.X + upPerson.HorCount + 1;
        ...
        private List<Person> TestData()
        {
            return new List<Person>()
            {
                ...
                new Person(){Id=22,Name="b2",Pid=1,Level=2,Sort=1},
                new Person(){Id=23,Name="b3",Pid=1,Level=2,Sort=1},
                ...
                new Person(){Id=33,Name="c3",Pid=21,Level=3,Sort=2},
                new Person(){Id=34,Name="c4",Pid=22,Level=3,Sort=2},
                new Person(){Id=35,Name="c5",Pid=22,Level=3,Sort=2},
                new Person(){Id=36,Name="c6",Pid=22,Level=3,Sort=2},
                new Person(){Id=37,Name="c7",Pid=23,Level=3,Sort=2},
                new Person(){Id=4,Name="d",Pid=3,Level=4,Sort=1},
                new Person(){Id=41,Name="d1",Pid=3,Level=4,Sort=1},
                new Person(){Id=42,Name="d2",Pid=33,Level=4,Sort=1},
                new Person(){Id=43,Name="d3",Pid=33,Level=4,Sort=1},
                new Person(){Id=44,Name="d4",Pid=35,Level=4,Sort=1},
                new Person(){Id=45,Name="d5",Pid=35,Level=4,Sort=1},
                new Person(){Id=5,Name="e",Pid=4,Level=5,Sort=1},
                new Person(){Id=51,Name="e1",Pid=41,Level=5,Sort=1},
                new Person(){Id=52,Name="e2",Pid=43,Level=5,Sort=1},
                new Person(){Id=53,Name="e3",Pid=43,Level=5,Sort=1},
                new Person(){Id=54,Name="e4",Pid=43,Level=5,Sort=1},
                new Person(){Id=55,Name="e5",Pid=45,Level=5,Sort=1},
            };
        }
